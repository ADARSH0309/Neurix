...
export async function listDriveFiles(
  ...
  const data = safeJsonParse(result.content[0].text!, {}) as Record<string, unknown>;
...
export async function listGmailMessages(
  ...
  const data = safeJsonParse(result.content[0].text!, {}) as Record<string, unknown>;
  return Array.isArray(data) ? data : (data.messages as GmailMessage[]) || [];
}
...
export async function listGmailLabels(
  ...
  const data = safeJsonParse(result.content[0].text!, {}) as Record<string, unknown>;
  return Array.isArray(data) ? data : (data.labels as Record<string, unknown>[]) || [];
}
