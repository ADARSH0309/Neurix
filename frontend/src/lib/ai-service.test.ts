...
describe('mcpToolsToOpenAI', () => {
  ...
  it('converts MCP tools to OpenAI format with server prefix', () => {
    ...
    expect((result[0] as any).function.name).toBe('gmail__send_message');
    expect((result[0] as any).function.description).toContain('[Gmail]');
  });
  ...
  it('handles multiple servers', () => {
    ...
    expect(result.map(r => (r as any).function.name)).toContain('gmail__send_message');
    expect(result.map(r => (r as any).function.name)).toContain('gdrive__list_files');
  });
