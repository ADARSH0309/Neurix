    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        if (url.pathname === '/oauth/callback' || url.pathname === '/oauth2callback') {
          ...
          console.log('✅ You can now run the Gmail MCP server!\n');
...
async function getNewTokenManual(oauth2Client: any, tokenPath: string): Promise<void> {
  const authUrl = oauth2Client.generateAuthUrl({
    access_type: 'offline',
    scope: SCOPES,
    prompt: 'consent',
  ...
  console.log('\n===========================================');
  console.log('Gmail OAuth Setup (Manual)');
  console.log('===========================================\n');
