...
export async function createHttpServer(config: McpServerConfig): Promise<Express> {
  ...
  app.get('/auth/gmail/callback', healthCheckCorsMiddleware, authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  app.get('/auth/g-mail/callback', healthCheckCorsMiddleware, authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  app.get('/gmail/callback', healthCheckCorsMiddleware, authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  ...
  app.get('/auth/gmail/callback', authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  app.get('/auth/g-mail/callback', authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  app.get('/gmail/callback', authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
 ...
 * Start HTTP server
 */
export async function startHttpServer(config: McpServerConfig, port: number = 8082): Promise<void> {
  const app = await createHttpServer(config);

  app.listen(port, () => {
    console.log(JSON.stringify({
      timestamp: new Date().toISOString(),
      message: 'Gmail MCP Server started (Phase 3 - SSE Support)',
      port,
      transport: 'http',
