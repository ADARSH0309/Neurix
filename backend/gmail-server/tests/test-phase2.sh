...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Google Gmail MCP Server - Phase 2 Tests${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
run_test "Invalid JSON-RPC version returns error" \
  "curl -s -X POST $BASE_URL/ -H 'Content-Type: application/json' -H 'Cookie: neurix_gmail_session=fake' -d '{\"jsonrpc\":\"1.0\",\"id\":1,\"method\":\"initialize\"}'" \
  "32600"
...
run_test "Unknown method returns 404" \
  "curl -s -X POST $BASE_URL/ -H 'Content-Type: application/json' -H 'Cookie: neurix_gmail_session=fake-session-id' -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"unknown_method\"}'" \
  "32601"
