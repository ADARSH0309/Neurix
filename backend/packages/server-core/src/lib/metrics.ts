/**
 * Prometheus Metrics for Google Drive MCP Server
 *
...
export const toolCallCounter = new promClient.Counter({
  name: 'gdrive_mcp_tool_calls_total',
...
export const toolCallDuration = new promClient.Histogram({
  name: 'gdrive_mcp_tool_call_duration_seconds',
...
export const activeToolCalls = new promClient.Gauge({
  name: 'gdrive_mcp_active_tool_calls',
...
export const activeSessions = new promClient.Gauge({
  name: 'gdrive_mcp_active_sessions',
...
export const sessionEvents = new promClient.Counter({
  name: 'gdrive_mcp_session_events_total',
...
export const oauthEvents = new promClient.Counter({
  name: 'gdrive_mcp_oauth_events_total',
...
/**
 * Counter: Google Drive API calls
 */
export const gdriveApiCalls = new promClient.Counter({
  name: 'gdrive_mcp_gdrive_api_calls_total',
  help: 'Total Google Drive API calls',
  labelNames: ['operation', 'status'] as const,
  registers: [register],
});

/**
 * Histogram: Google Drive API latency
 */
export const gdriveApiLatency = new promClient.Histogram({
  name: 'gdrive_mcp_gdrive_api_latency_seconds',
  help: 'Google Drive API call latency in seconds',
...
export const circuitBreakerState = new promClient.Gauge({
  name: 'gdrive_mcp_circuit_breaker_state',
...
export const circuitBreakerEvents = new promClient.Counter({
  name: 'gdrive_mcp_circuit_breaker_events_total',
...
export const rateLimitEvents = new promClient.Counter({
  name: 'gdrive_mcp_rate_limit_events_total',
...
export const httpRequestsTotal = new promClient.Counter({
  name: 'gdrive_mcp_http_requests_total',
...
export const httpRequestDuration = new promClient.Histogram({
  name: 'gdrive_mcp_http_request_duration_seconds',
...
/**
 * Record Google Drive API call metrics
 */
export function recordGdriveApiCall(
  operation: string,
  status: 'success' | 'error',
  durationMs: number
): void {
  gdriveApiCalls.inc({ operation, status });
  gdriveApiLatency.observe({ operation }, durationMs / 1000);
}
