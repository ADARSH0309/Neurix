...
export const corsMiddleware = cors({
  origin: (origin, callback) => {
    ...
    // Format: https://gcalendar-mcp.daffyos.in
    if (origin.startsWith('https://gcalendar-mcp.daffyos.in') ||
        origin.startsWith('http://gcalendar-mcp.daffyos.in')) {
      return callback(null, true);
    }

    // Allow gcalendar-agent-dev domain
    if (origin.startsWith('https://gcalendar-agent-dev.daffyos.in') ||
        origin.startsWith('http://gcalendar-agent-dev.daffyos.in')) {
      return callback(null, true);
    }
...
// MCP requests: 1MB limit (for JSON-RPC requests with calendar content)
