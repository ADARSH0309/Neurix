 ...
 * Each authenticated session gets its own GCalendarClient and MCP adapter instance.
 */
...
/**
 * OAuth config and client for creating GCalendarClient instances and refreshing tokens
 */
let oauthConfig: {
  clientId: string;
  clientSecret: string;
...
/**
 * Create GCalendarClient from OAuth tokens
 */
function createCalendarClientFromTokens(tokens: OAuthTokens): GCalendarClient {
  if (!oauthConfig) {
    throw new Error('OAuth config not initialized');
  }

  const client = new GCalendarClient(
...
export async function handleSessionMcpRequest(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    // Create per-session GCalendarClient and MCP adapter with fresh tokens
    const calendarClient = createCalendarClientFromTokens(freshTokens);
    const mcpAdapter = new McpHttpAdapter(calendarClient);
