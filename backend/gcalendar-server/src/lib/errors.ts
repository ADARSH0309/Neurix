/**
 * Custom Error Classes for Google Calendar MCP Server
 *
 ...
 * Wraps Google Calendar-specific API errors
 */
export class GcalendarApiError extends McpError {
  constructor(
    message: string,
    public apiErrorCode?: number,
    public apiErrorReason?: string
  ) {
    const statusCode = apiErrorCode || 500;
    super(message, statusCode, 'GCALENDAR_API_ERROR');
  }
}
...
export function isRetryableError(error: unknown): boolean {
  ...
  if (error instanceof GcalendarApiError) {
    return [429, 500, 502, 503, 504].includes(error.apiErrorCode || 0);
  }
  return false;
}
