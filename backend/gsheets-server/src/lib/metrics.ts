/**
 * Prometheus Metrics for Google Sheets MCP Server
 *
...
export const toolCallCounter = new promClient.Counter({
  name: 'gsheets_mcp_tool_calls_total',
...
export const toolCallDuration = new promClient.Histogram({
  name: 'gsheets_mcp_tool_call_duration_seconds',
...
export const activeToolCalls = new promClient.Gauge({
  name: 'gsheets_mcp_active_tool_calls',
...
export const activeSessions = new promClient.Gauge({
  name: 'gsheets_mcp_active_sessions',
...
export const sessionEvents = new promClient.Counter({
  name: 'gsheets_mcp_session_events_total',
...
export const oauthEvents = new promClient.Counter({
  name: 'gsheets_mcp_oauth_events_total',
 ...
 */
export const gsheetsApiCalls = new promClient.Counter({
  name: 'gsheets_mcp_gsheets_api_calls_total',
  help: 'Total Google Sheets API calls',
  labelNames: ['operation', 'status'] as const,
  registers: [register],
});

/**
 * Histogram: Google Sheets API latency
 */
export const gsheetsApiLatency = new promClient.Histogram({
  name: 'gsheets_mcp_gsheets_api_latency_seconds',
  help: 'Google Sheets API call latency in seconds',
...
export const circuitBreakerState = new promClient.Gauge({
  name: 'gsheets_mcp_circuit_breaker_state',
...
export const circuitBreakerEvents = new promClient.Counter({
  name: 'gsheets_mcp_circuit_breaker_events_total',
...
export const rateLimitEvents = new promClient.Counter({
  name: 'gsheets_mcp_rate_limit_events_total',
...
export const httpRequestsTotal = new promClient.Counter({
  name: 'gsheets_mcp_http_requests_total',
...
export const httpRequestDuration = new promClient.Histogram({
  name: 'gsheets_mcp_http_request_duration_seconds',
...
/**
 * Record Google Sheets API call metrics
 */
export function recordGsheetsApiCall(
  operation: string,
  status: 'success' | 'error',
  durationMs: number
): void {
  gsheetsApiCalls.inc({ operation, status });
  gsheetsApiLatency.observe({ operation }, durationMs / 1000);
}
