...
/**
 * Google Sheets MCP Server - Entry point
 *
...
import { GSheetsServer } from './server.js';
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        if (pathsToMatch.includes(url.pathname)) {
          ...
          res.end(`<html><body style="font-family:system-ui;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;background:linear-gradient(135deg,#34d399 0%,#059669 100%)"><div style="text-align:center;background:white;padding:40px;border-radius:12px;box-shadow:0 10px 40px rgba(0,0,0,0.2)"><div style="font-size:64px;margin-bottom:20px">✅</div><h1 style="color:#059669;margin:0 0 10px 0">Authentication Successful!</h1><p style="color:#666;margin:0">Google Sheets MCP Server is now connected.</p><p style="color:#999;margin-top:20px;font-size:14px">You can close this window.</p></div></body></html>`);
          server.close();
          resolve();
        } else {
    ...
    server.listen(port, () => {
      ...
      console.error(JSON.stringify({
        ...
        level: 'info',
        message: 'Opening browser for Google Sheets authentication...',
      }));
...
async function main() {
  ...
  try {
    ...
    const server = new GSheetsServer(clientId, clientSecret, redirectUri, tokenPath);
    ...
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'info',
      message: 'Google Sheets MCP Server started successfully',
    }));
  } catch (error) {
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'error',
      message: 'Failed to start Google Sheets MCP server',
...
process.on('SIGINT', () => {
  console.error(JSON.stringify({
    timestamp: new Date().toISOString(),
    level: 'info',
    message: 'Shutting down Google Sheets MCP server...',
  }));
  process.exit(0);
});
