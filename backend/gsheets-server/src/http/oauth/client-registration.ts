...
export class ClientRegistrationManager {
  private redis: Redis;
  private keyPrefix = 'gsheets:oauth:client:';
  private isConnected: boolean = false;

  constructor() {
    this.redis = getRedisClient();
    this.redis.on('error', (err: Error) => {
      console.error(JSON.stringify({ timestamp: new Date().toISOString(), level: 'error', message: 'Redis client error in ClientRegistrationManager', error: err.message }));
    });
  }

  async initialize(): Promise<void> {
    this.isConnected = true;
    console.log(JSON.stringify({ timestamp: new Date().toISOString(), level: 'info', message: 'ClientRegistrationManager initialized' }));
  }

  async registerClient(request: ClientRegistrationRequest): Promise<RegisteredClient> {
    const client_id = `mcp_gsheets_${randomBytes(16).toString('hex')}`;
