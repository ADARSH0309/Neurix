...
export async function createHttpServer(config: McpServerConfig): Promise<Express> {
  ...
  app.get('/sheets/callback', authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
 ...
 * Start HTTP server
 */
export async function startHttpServer(config: McpServerConfig, port: number = 8085): Promise<void> {
  const app = await createHttpServer(config);

  app.listen(port, () => {
    console.log(JSON.stringify({
      timestamp: new Date().toISOString(),
      message: 'Google Sheets MCP Server started (Phase 3 - SSE Support)',
      port,
      transport: 'http',
