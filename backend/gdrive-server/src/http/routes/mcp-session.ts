 ...
 * Each authenticated session gets its own GDriveClient and MCP adapter instance.
 */
...
/**
 * OAuth config and client for creating GDriveClient instances and refreshing tokens
 */
let oauthConfig: {
  clientId: string;
  clientSecret: string;
...
/**
 * Create GDriveClient from OAuth tokens
 */
function createDriveClientFromTokens(tokens: OAuthTokens): GDriveClient {
  if (!oauthConfig) {
...
export async function handleSessionMcpRequest(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    // Create per-session GDriveClient and MCP adapter with fresh tokens
    const driveClient = createDriveClientFromTokens(freshTokens);
    const mcpAdapter = new McpHttpAdapter(driveClient);
