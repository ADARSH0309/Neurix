...
export async function createHttpServer(config: McpServerConfig): Promise<Express> {
  ...
  app.get('/auth/g-drive/callback', healthCheckCorsMiddleware, authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  app.get('/drive/callback', healthCheckCorsMiddleware, authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
...
export async function startHttpServer(config: McpServerConfig, port: number = 3000): Promise<void> {
  ...
  app.listen(port, () => {
    console.log(JSON.stringify({
      timestamp: new Date().toISOString(),
      message: 'Google Drive MCP Server started (Phase 3 - SSE Support)',
      port,
      transport: 'http',
