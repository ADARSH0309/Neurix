...
import { GDriveServer } from './server.js';
            ...
            <html>
              <body style="font-family: system-ui; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                <div style="text-align: center; background: white; padding: 40px; border-radius: 12px; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
                  ...
                  <p style="color: #666; margin: 0;">Google Drive MCP Server is now connected.</p>
...
async function main() {
  ...
  try {
    ...
    // Start the server
    const server = new GDriveServer(clientId, clientSecret, redirectUri, tokenPath);
    ...
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'info',
      message: 'Google Drive MCP Server started successfully',
    }));
  } catch (error) {
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'error',
      message: 'Failed to start Google Drive MCP server',
...
process.on('SIGINT', () => {
  console.error(JSON.stringify({
    timestamp: new Date().toISOString(),
    level: 'info',
    message: 'Shutting down Google Drive MCP server...',
  }));
  process.exit(0);
});
