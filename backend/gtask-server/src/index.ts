...
/**
 * Google Tasks MCP Server - Entry point
 *
...
import { GTaskServer } from './server.js';
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, () => {
      ...
      console.error(JSON.stringify({
        ...
        level: 'info',
        message: 'Opening browser for Google Tasks authentication...',
      }));
...
async function main() {
  ...
  const redirectUri = process.env.GOOGLE_REDIRECT_URI || 'http://localhost:8084/auth/g-task/callback';
  ...
  try {
    ...
    const server = new GTaskServer(clientId, clientSecret, redirectUri, tokenPath);
    ...
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'info',
      message: 'Google Tasks MCP Server started successfully',
    }));
  } catch (error) {
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'error',
      message: 'Failed to start Google Tasks MCP server',
...
process.on('SIGINT', () => {
  console.error(JSON.stringify({
    timestamp: new Date().toISOString(),
    level: 'info',
    message: 'Shutting down Google Tasks MCP server...',
  }));
  process.exit(0);
});
