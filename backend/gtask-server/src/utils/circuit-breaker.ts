/**
 * Circuit Breaker for Google Tasks API
 *
...
export function formatCircuitBreakerError(error: unknown, apiName: string): string {
  if (isCircuitBreakerOpenError(error)) {
    return `Google Tasks API is currently unavailable (${apiName}). The circuit breaker is open due to repeated failures. Please try again in 30 seconds.`;
  }
  if (isCircuitBreakerTimeoutError(error)) {
    return `Google Tasks API request timed out (${apiName}). The request took longer than 10 seconds. Please try again.`;
  }
  if (error instanceof Error) {
    return `Google Tasks API error (${apiName}): ${error.message}`;
  }
  return `Google Tasks API error (${apiName}): Unknown error`;
}
