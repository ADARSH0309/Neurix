...
export async function createHttpServer(config: McpServerConfig): Promise<Express> {
  ...
  app.get('/auth/g-task/callback', healthCheckCorsMiddleware, authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  app.get('/g-task/callback', healthCheckCorsMiddleware, authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
  ...
  app.get('/g-task/callback', authLimiter, validateQuery(schemas.oauthCallback), handleOAuthCallback);
 ...
 * Start HTTP server
 */
export async function startHttpServer(config: McpServerConfig, port: number = 8084): Promise<void> {
  const app = await createHttpServer(config);

  app.listen(port, () => {
    console.log(JSON.stringify({
      timestamp: new Date().toISOString(),
      message: 'Google Tasks MCP Server started (Phase 3 - SSE Support)',
      port,
      transport: 'http',
