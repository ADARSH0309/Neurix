 ...
 * Each authenticated session gets its own GTaskClient and MCP adapter instance.
 */
...
/**
 * OAuth config and client for creating GTaskClient instances and refreshing tokens
 */
let oauthConfig: {
  clientId: string;
  clientSecret: string;
...
/**
 * Create GTaskClient from OAuth tokens
 */
function createTaskClientFromTokens(tokens: OAuthTokens): GTaskClient {
  if (!oauthConfig) {
    throw new Error('OAuth config not initialized');
  }

  const client = new GTaskClient(
...
export async function handleSessionMcpRequest(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    // Create per-session GTaskClient and MCP adapter with fresh tokens
    const taskClient = createTaskClientFromTokens(freshTokens);
    const mcpAdapter = new McpHttpAdapter(taskClient);
