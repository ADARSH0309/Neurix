/**
 * Zod Validation Schemas for Google Tasks MCP Server
 */
 ...
 */
export const taskListIdSchema = z.string().min(1, 'Task list ID required').max(256, 'Task list ID too long');

export const listTasksParamsSchema = z.object({
  taskListId: taskListIdSchema.optional().default('@default'),
  maxResults: z.number().int().min(1).max(100).default(20),
  showCompleted: z.boolean().optional().default(true),
  showDeleted: z.boolean().optional().default(false),
  showHidden: z.boolean().optional().default(false),
  dueMin: z.string().max(100).optional(),
  dueMax: z.string().max(100).optional(),
  pageToken: z.string().max(256).optional(),
});

export const createTaskParamsSchema = z.object({
  taskListId: taskListIdSchema.optional().default('@default'),
  title: z.string().min(1, 'Title required').max(1000, 'Title too long'),
  notes: z.string().max(5000, 'Notes too long').optional(),
  due: z.string().max(100).optional(),
  status: z.enum(['needsAction', 'completed']).optional(),
  parent: z.string().max(256).optional(),
  previous: z.string().max(256).optional(),
});
...
export const schemas = {
  ...
  listTasksParams: listTasksParamsSchema,
