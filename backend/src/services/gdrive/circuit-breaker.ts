 ...
 * when Drive APIs slow down or become unavailable.
 *
...
export interface DriveCircuitBreakerOptions {
...
const DEFAULT_OPTIONS: Required<Omit<DriveCircuitBreakerOptions, 'name'>> = {
  timeout: 10000, // 10 seconds (Drive operations can be slower)
...
export function createDriveCircuitBreaker<T extends (...args: any[]) => Promise<any>>(
  driveApiFunction: T,
  options: DriveCircuitBreakerOptions = {},
  onStateChange?: CircuitBreakerStateCallback
): CircuitBreaker<any[], any> {
  const config = { ...DEFAULT_OPTIONS, ...options };
  const name = options.name || 'DriveAPI';

  const breaker = new CircuitBreaker(driveApiFunction, {
...
export function formatCircuitBreakerError(error: unknown, apiName: string): string {
  if (isCircuitBreakerOpenError(error)) {
    return `Google Drive API is currently unavailable (${apiName}). The circuit breaker is open due to repeated failures. Please try again in 30 seconds.`;
  }
  if (isCircuitBreakerTimeoutError(error)) {
    return `Google Drive API request timed out (${apiName}). The request took longer than 10 seconds. Please try again.`;
  }
  if (error instanceof Error) {
    return `Google Drive API error (${apiName}): ${error.message}`;
  }
  return `Google Drive API error (${apiName}): Unknown error`;
}
