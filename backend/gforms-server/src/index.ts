...
import { GFormsServer } from './server.js';
...
async function performOAuth(
  ...
  return new Promise((resolve, reject) => {
    ...
    const port = parseInt(redirectUrl.port) || 8084;
    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        if (url.pathname === callbackPath || url.pathname === '/oauth/callback' || url.pathname === '/oauth2callback') {
            ...
            <html>
              <body style="font-family: system-ui; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                <div style="text-align: center; background: white; padding: 40px; border-radius: 12px; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
                  ...
                  <p style="color: #666; margin: 0;">Google Forms MCP Server is now connected.</p>
...
async function main() {
  ...
  const redirectUri = process.env.GOOGLE_REDIRECT_URI || 'http://localhost:8084/auth/google-forms/callback';
  ...
  if (!clientId || !clientSecret) {
    ...
    console.error('  GOOGLE_REDIRECT_URI=... (optional, defaults to http://localhost:8084/auth/google-forms/callback)');
  ...
  try {
    ...
    // Start the server
    const server = new GFormsServer(clientId, clientSecret, redirectUri, tokenPath);
    ...
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'info',
      message: 'Google Forms MCP Server started successfully',
    }));
  } catch (error) {
    console.error(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'error',
      message: 'Failed to start Google Forms MCP server',
...
process.on('SIGINT', () => {
  console.error(JSON.stringify({
    timestamp: new Date().toISOString(),
    level: 'info',
    message: 'Shutting down Google Forms MCP server...',
  }));
  process.exit(0);
});
