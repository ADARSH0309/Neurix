 ...
 * Each authenticated session gets its own GFormsClient and MCP adapter instance.
 */
...
/**
 * OAuth config and client for creating GFormsClient instances and refreshing tokens
 */
let oauthConfig: {
  clientId: string;
  clientSecret: string;
 ...
 *
 * Phase 5.1 - CRITICAL Security: Input validation performed by Zod middleware (jsonRpcRequestSchema)
...
export async function handleSessionMcpRequest(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    // Create per-session GFormsClient and MCP adapter with fresh tokens
    const gformsClient = createGFormsClientFromTokens(freshTokens);
