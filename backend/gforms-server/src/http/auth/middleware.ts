 ...
 * 1. Bearer Token (for MCP Inspector) - Authorization: Bearer <token>
 * 2. Session Cookie (for web UI) - neurix_gforms_session cookie
 *
 * Priority: Bearer token is checked first, then falls back to cookie.
 * This ensures MCP Inspector works while maintaining backward compatibility.
 */
...
async function authenticateWithBearerToken(req: Request): Promise<AuthResult> {
  ...
  if (!validation.valid) {
    ...
    // Security audit log for failed authentication attempt
  ...
  // Get session from token's session ID
  ...
  if (!session || !session.authenticated) {
    ...
    // Phase 5.1 - Week 2, Task 2.2: Security Audit Logging (Issue #7)
    // Security audit log for failed authentication due to invalid session
    console.log(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'security',
      event: 'authentication_failed',
...
async function authenticateWithCookie(req: Request): Promise<AuthResult> {
  ...
  if (!session || !session.authenticated) {
    ...
    // Phase 5.1 - Week 2, Task 2.2: Security Audit Logging (Issue #7)
    // Security audit log for failed cookie authentication
    console.log(JSON.stringify({
      timestamp: new Date().toISOString(),
      level: 'security',
      event: 'authentication_failed',
 ...
 * Attaches session data to req.session if authenticated.
 *
 * Usage:
...
export function requireAuth() {
  return async (
    ...
    try {
      ...
      // If bearer token fails, try cookie authentication
      ...
      if (!authResult.success) {
        // Phase 5.1 - Week 2, Task 2.2: Security Audit Logging (Issue #7)
        // Security audit log for overall authentication failure
        console.log(JSON.stringify({
          timestamp: new Date().toISOString(),
          level: 'security',
          event: 'authentication_failed',
 ...
 * Useful for endpoints that have optional authenticated features.
 */
export function optionalAuth() {
  return async (
    ...
    try {
      ...
      // If authenticated, attach session data
      if (authResult.success) {
        req.session = authResult.session;
        req.sessionId = authResult.sessionId;
        req.authMethod = authResult.method;
...
/**
 * Get authentication info from request (for debugging)
 */
