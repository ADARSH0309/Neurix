 ...
 * Manages OAuth client registrations for MCP Inspector and other dynamic clients
 */
...
export class ClientRegistrationManager {
  private redis: Redis;
  private keyPrefix = 'oauth:client:';
  ...
  async registerClient(request: ClientRegistrationRequest): Promise<RegisteredClient> {
    ...
    // For public clients (MCP Inspector), no client_secret needed
    ...
    // Store in Redis (expire after 30 days)
   ...
   *
   * Security: Requires exact match for protocol, host, port, and path.
   * For localhost development, we require the full URI to be registered.
   */
  async validateRedirectUri(client_id: string, redirect_uri: string): Promise<boolean> {
    ...
    // Require exact match - no wildcards or port variations allowed
