...
export class SessionManager implements ISessionManager {
  ...
  async getSession(sessionId: string): Promise<Session | null> {
    ...
    try {
      ...
      // Get remaining TTL and use setex (Redis 5.x compatible - KEEPTTL requires Redis 6.0+)
  ...
  async updateSession(sessionId: string, updates: Partial<Session>): Promise<Session | null> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        // Step 4: Get TTL and MULTI/EXEC to atomically SET new value
        // If key was modified since WATCH, transaction will fail and return null
        // Note: Using setex instead of set...KEEPTTL for Redis 5.x compatibility
