/**
 * Prometheus Metrics for Google Forms MCP Server
 *
...
export const toolCallCounter = new promClient.Counter({
  name: 'gforms_mcp_tool_calls_total',
...
export const toolCallDuration = new promClient.Histogram({
  name: 'gforms_mcp_tool_call_duration_seconds',
...
export const activeToolCalls = new promClient.Gauge({
  name: 'gforms_mcp_active_tool_calls',
...
export const activeSessions = new promClient.Gauge({
  name: 'gforms_mcp_active_sessions',
...
export const sessionEvents = new promClient.Counter({
  name: 'gforms_mcp_session_events_total',
...
export const oauthEvents = new promClient.Counter({
  name: 'gforms_mcp_oauth_events_total',
...
// ============================================================================
// Google Forms API Metrics
// ============================================================================

/**
 * Counter: Google Forms API calls
 */
export const gformsApiCalls = new promClient.Counter({
  name: 'gforms_mcp_gforms_api_calls_total',
  help: 'Total Google Forms API calls',
  labelNames: ['operation', 'status'] as const,
  registers: [register],
});

/**
 * Histogram: Google Forms API latency
 */
export const gformsApiLatency = new promClient.Histogram({
  name: 'gforms_mcp_gforms_api_latency_seconds',
  help: 'Google Forms API call latency in seconds',
...
export const circuitBreakerState = new promClient.Gauge({
  name: 'gforms_mcp_circuit_breaker_state',
...
export const circuitBreakerEvents = new promClient.Counter({
  name: 'gforms_mcp_circuit_breaker_events_total',
...
export const rateLimitEvents = new promClient.Counter({
  name: 'gforms_mcp_rate_limit_events_total',
...
export const httpRequestsTotal = new promClient.Counter({
  name: 'gforms_mcp_http_requests_total',
...
export const httpRequestDuration = new promClient.Histogram({
  name: 'gforms_mcp_http_request_duration_seconds',
...
/**
 * Record Google Forms API call metrics
 */
export function recordGformsApiCall(
  operation: string,
  status: 'success' | 'error',
  durationMs: number
): void {
  gformsApiCalls.inc({ operation, status });
  gformsApiLatency.observe({ operation }, durationMs / 1000);
}
